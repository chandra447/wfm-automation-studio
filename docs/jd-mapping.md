...
| An agent that edits a graph, not a document | The builder chat proposes operations from a closed set, applied server-side by one validator-backed applier. The client sends its current graph every turn, a refused operation explains the legal alternatives, and an edit that would leave the graph invalid is dropped whole | `packages/workflows/tests/builder-operations.test.ts`, `scripts/verify-features.sh` step 13, `docs/adr/0013-conversational-builder.md` |
