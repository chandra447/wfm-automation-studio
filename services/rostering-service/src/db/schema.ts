import { pgTable, integer, jsonb, pgEnum, primaryKey, text, timestamp, uuid } from 'drizzle-orm/pg-core';

/**
 * Rostering persistence. This file is the model drizzle-kit generates
 * migrations from (drizzle.config.ts → drizzle/); query code uses postgres.js
 * against the same columns because the outbox and every transactional write
 * share its transaction type (ADR-0007).
 */
...
export const shifts = pgTable('shifts', {
  ...
  requiredQualificationCodes: jsonb('required_qualification_codes').$type<string[]>().notNull(),
